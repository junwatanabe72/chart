...
const StockChart: React.FC = () => {
  ...
  // クロスヘアーの状態
  const [crosshairValues, setCrosshairValues] = useState<{x: number, y: number, xValue: string, yValue: number} | null>(null);
  const [showCrosshair, setShowCrosshair] = useState<boolean>(true);

  // 描画ツールの状態
  const [drawingMode, setDrawingMode] = useState<'trendline' | 'horizontalline' | null>(null);
  const [drawingLines, setDrawingLines] = useState<DrawingLine[]>([]);
  const [drawingStart, setDrawingStart] = useState<{ index: number, price: number } | null>(null);
  const [drawingEnd, setDrawingEnd] = useState<{ index: number, price: number } | null>(null);
  const [selectedLine, setSelectedLine] = useState<string | null>(null);
  const [lineColors] = useState<string[]>(['#FF5722', '#2196F3', '#4CAF50', '#9C27B0', '#E91E63']);
  ...
  // RSIを計算する関数
  const calculateRSI = (data: StockData[], period: number = 14): (number | null)[] => {
  ...
  const calculateMACD = (
    ...
    const calculateEMA = (prices: number[], period: number): (number | null)[] => {
      ...
      for (let i = period; i < prices.length; i++) {
        ema.push((prices[i] - (ema[i-1] as number)) * multiplier + (ema[i-1] as number));
      }
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    if (drawingMode) {
      ...
      const visibleEndIndex = xAxisDomain ? Math.ceil(xAxisDomain[1]) : stockData.length - 1;
      const visibleRange = visibleEndIndex - visibleStartIndex;

      const indexPosition = visibleStartIndex + (relativeX / chartWidth) * visibleRange;
  ...
  const renderDrawingLines = () => {
    ...
    return allLines.map(line => {
      // インデックスをX座標に変換
      const startX = (line.startIndex - (xAxisDomain ? xAxisDomain[0] : 0)) /
                    ((xAxisDomain ? xAxisDomain[1] : stockData.length - 1) - (xAxisDomain ? xAxisDomain[0] : 0)) * 100;
      const endX = (line.endIndex - (xAxisDomain ? xAxisDomain[0] : 0)) /
                  ((xAxisDomain ? xAxisDomain[1] : stockData.length - 1) - (xAxisDomain ? xAxisDomain[0] : 0)) * 100;
      ...
      return (
        <g key={line.id} onClick={() => setSelectedLine(line.id)}>
          ...
          {isSelected && (
            <>
              <circle cx={`${startX}%`} cy={`${startY}%`} r={4} fill={line.color} />
              ...
              <g
                ...
                style={{ cursor: 'pointer' }}
              >
                <rect x="-10" y="-10" width="20" height="20" fill="white" opacity="0.8" rx="2" />
                <text x="0" y="4" textAnchor="middle" fill="red" fontSize="14">×</text>
              </g>
            </>
          )}
        </g>
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-lg">
          ...
          {indicators.rsi && data.rsi !== null && (
            <p className={`text-sm font-medium ${(data.rsi as number) > 70 ? 'text-red-600' : (data.rsi as number) < 30 ? 'text-green-600' : 'text-gray-600'}`}>
              RSI: {(data.rsi as number).toFixed(2)}
            </p>
          )}

          {/* MACD値 */}
          {indicators.macd && data.macdLine !== null && data.signalLine !== null && (
            <>
              <p className="text-sm text-gray-600">
                MACD: <span className="font-medium" style={{ color: '#2196F3' }}>{(data.macdLine as number).toFixed(2)}</span>
              </p>
              <p className="text-sm text-gray-600">
                Signal: <span className="font-medium" style={{ color: '#FF5722' }}>{(data.signalLine as number).toFixed(2)}</span>
              </p>
              <p className={`text-sm font-medium ${(data.macdHistogram as number) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
...
/>
    ...
    return (
      <div className="h-32 mb-6">
        <h3 className="text-sm font-medium mb-1">
          MACD ({macdParams.fastPeriod}, {macdParams.slowPeriod}, {macdParams.signalPeriod})
        </h3>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            data={visibleData}
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', '+', '-',
           'c', 't', 'h', 'r', 'm', 'b', '1', '2', '3', '4', '?'].includes(e.key)) {
        e.preventDefault();
      }
      ...
      if (e.key === 'ArrowLeft') {
        ...
        if (xAxisDomain) {
          const domainRange = xAxisDomain[1] - xAxisDomain[0];
          const panAmount = domainRange * 0.1;
          const newEnd = Math.min(stockData.length - 1, xAxisDomain[1] + panAmount);
        ...
        // 水平線描画モード切替
        toggleDrawingMode(drawingMode === 'horizontalline' ? null : 'horizontalline');
      } else if (e.key === 'r') {
        // RSI表示切替
        ...
        // ボリンジャーバンド表示切替
        setIndicators(prev => ({...prev, bollingerBands: !prev.bollingerBands}));
      } else if (e.key === '1') {
        // ローソク足に切替
  ...
  }, [xAxisDomain, stockData.length, showCrosshair, drawingMode, indicators, chartStyle, showKeyboardShortcuts]);
  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4">Stock {stockData[0]?.code || "130A0"} チャート</h2>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded border border-gray-200">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={`px-3 py-1 text-sm rounded ${chartStyle === 'candlestick' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            ローソク足
          </button>
          <button
            onClick={() => setChartStyle('ohlc')}
            className={`px-3 py-1 text-sm rounded ${chartStyle === 'ohlc' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            OHLC
          </button>
          <button
            onClick={() => setChartStyle('line')}
            className={`px-3 py-1 text-sm rounded ${chartStyle === 'line' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            ライン
          </button>
          <button
            onClick={() => setChartStyle('area')}
            className={`px-3 py-1 text-sm rounded ${chartStyle === 'area' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            エリア
          </button>
        </div>
      ...
      <div className="mb-4 p-3 bg-gray-50 rounded border border-gray-200">
        <div className="flex flex-wrap items-center gap-3 mb-2">
          ...
          <button
            ...
            className={`px-3 py-1 text-sm rounded ${drawingMode === 'trendline' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            トレンドライン
          </button>
          <button
            onClick={() => toggleDrawingMode('horizontalline')}
            className={`px-3 py-1 text-sm rounded ${drawingMode === 'horizontalline' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          >
            水平線
          </button>
            ...
            <div className="flex items-center ml-2 space-x-2">
              <div className="flex items-center">
                ...
                <select
                  ...
                  onChange={(e) => setMacdParams({...macdParams, fastPeriod: Number(e.target.value)})}
              ...
              <div className="flex items-center">
                ...
                <select
                  ...
                  onChange={(e) => setMacdParams({...macdParams, slowPeriod: Number(e.target.value)})}
                  className="form-select text-sm border border-gray-300 rounded p-1 w-12"
                >
                  <option value="21">21</option>
              ...
              <div className="flex items-center">
                ...
                <select
                  ...
                  onChange={(e) => setMacdParams({...macdParams, signalPeriod: Number(e.target.value)})}
                  className="form-select text-sm border border-gray-300 rounded p-1 w-12"
                >
      ...
      <div className="h-32 mb-6">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Tooltip content={<CustomTooltip />} />
            <Bar dataKey="volume" name="出来高" id={`volume-chart-${Date.now()}`}>
              {visibleData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
      ...
      <div className="h-20">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Bar dataKey="volume" fill="#CBD5E1" fillOpacity={0.5} id={`volume-chart-${Date.now()}`}/>
          </BarChart>
        </ResponsiveContainer>
      </div>
